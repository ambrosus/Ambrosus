...
class Measurement {
    ...
    async _signedHash() {        
        ...
        return await this._signWithNode(abi.soliditySHA3(TYPES, values).toString('hex'), this.device);
    }


    async _signWithNode(text, device){
    ...
    async encode() {
        ...
        let values = [ this.attribute_id, this.value, this.event_id, this.timestamp, this.farmer_id, this.batch_id, this.device].concat(await this._signedHash());    
        return TYPES.map( (type, i) => this._normalizeLength(values[i], type));
    }

    _normalizeLength(value, type){
        if (type=="address"){
