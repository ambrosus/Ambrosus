...
class TokenSingleton {
  ...
  create(addresses = [web3.eth.accounts[0]], amounts = [0], transactionHashCallback) {
    ...
    return new Promise((resolve, reject) => {
      this.TokenContract.new(addresses, amounts.map(val => utils.toBigNumberWithDecimals(val, DECIMALS)), transactionParams,
  ...
  async balanceOf(address = web3.eth.accounts[0]) {
    return utils.fromBigNumberWithDecimals(await promisify(this.contract.balanceOf)(address), DECIMALS);
  }

  async chargeMyAccount(amount, transactionHashCallback) {
    var tx = await promisify(this.contract.chargeMyAccount)(utils.toBigNumberWithDecimals(amount, DECIMALS), {from: web3.eth.accounts[0], gas: DEPLOY_GAS_LIMIT});
