...
class TokenSingleton {
  ...
  async balanceOf(address = web3.eth.accounts[0]) {
    return web3.toDecimal(await promisify(this.contract.balanceOf)(address));
  }

  async chargeMyAccount(amount, transactionHashCallback) {
    var tx = await promisify(this.contract.chargeMyAccount)(amount, {from: web3.eth.accounts[0], gas: DEPLOY_GAS_LIMIT});
