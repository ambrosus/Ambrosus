  ...
  initiateAgreement(offerAddress, quantity, transactionHashCallback) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await promisify(token.approve)(this.marketAddress, 
              await promisify(marketContract.calculatePrice)(offerAddress, quantity),
              { from: web3.eth.accounts[0] });
      } catch (e) {
        reject(e);
      }
      marketContract.buy(offerAddress, quantity, transactionParams,
        async (err, tx) => {
          if (err) {
            ...
            var agreementAddress = await promisify(marketContract.getNewestAgreement)();
            resolve(new Agreement(this.AgreementContract.at(agreementAddress), this.marketAddress));
          }
        });
    });
  }
