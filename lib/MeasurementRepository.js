...
class MeasurementRepository {
  ...
  async create(devices, transactionHashCallback) {
    ...
    return new Promise((resolve, reject) => {
      this.MeasurementFactoryContract.new(devices, ipfsMap.getOwnHash(), transactionParams, async (err, newContract) => {
          if (err) {
            reject(err);
  ...
  async fromAddress(measurementsAddress) {
    ...
    return new MeasurementsStorage(await IPFSMap.createFromHash(this.ipfs, multihash), measurementsAddress);
  }
}
