...
class OfferRepository {
  ...
  async save(marketAddress, offer, txMinedCallback) {
    ...
    return new Promise((resolve, reject) => {
      this.OfferContract.new(
        ...
        tx, (err, newContract) => {
          if (err) {
            reject(err);
          } else if (!newContract.address && txMinedCallback) {
            txMinedCallback(newContract.transactionHash);
          } else if (newContract.address) {
            return resolve(newContract);
          }
        });
    });
  }
